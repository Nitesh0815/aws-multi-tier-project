...
resource "aws_security_group" "app" {
  ...
  ingress {
    ...
    to_port   = 22
    protocol  = "tcp"
    # FIXED: Security groups can't reference each other across VPCs without peering. 
    # Using MGMT VPC CIDR block instead.
...
resource "aws_launch_template" "app" {
  ...
  user_data = base64encode(<<-EOF
    ...
    # Create a simple HTML page that queries RDS
    cat > /var/www/html/index.html <<INNEREOF
    <html>
    <body>
    <h1>Hello from AWS Multi-Tier App!</h1>
    <p>Static asset from S3: <img src="https://${var.s3_bucket_name}.s3.amazonaws.com/sample.jpg" alt="S3 Image"></p>
    <p>RDS Connection Test: $(mysql -h ${aws_db_instance.prod_mysql.endpoint} -u ${var.db_username} -p${var.db_password} -e "SELECT 'Database Connected!' AS Status;" 2>/dev/null | tail -1)</p>
    </body>
    </html>
    INNEREOF
  EOF
...
resource "aws_s3_bucket" "static_assets" {
  # REMINDER: Ensure var.s3_bucket_name in terraform.tfvars is globally unique
...
resource "aws_s3_object" "sample" {
  ...
  key    = "sample.jpg"
  source = "sample.jpg" # Ensure this file exists in your local folder!
}
...
resource "aws_sns_topic_subscription" "email" {
  ...
  protocol  = "email"
  # FIXED: Replace placeholder with a valid email string
